...
String mapOldPackagePath(String pkgPath) {
  ...
  if (entityRegex.hasMatch(pkgPath)) {
    ...
    final String entityName = match.group(1)!;
    final String ext = match.group(2)!;
    final String featureName = getFeatureNameForEntity(entityName);
  ...
  if (serviceRegex.hasMatch(pkgPath)) {
    ...
    final String sName = match.group(1)!;
    String featureName = '';
  ...
  if (providerRegex.hasMatch(pkgPath)) {
    ...
    final String pName = match.group(1)!;
    String featureName = '';
...
String resolveRelativePath(String currentFilePath, String relativeImport) {
  ...
  final List<String> parts = currentFilePath.split('/');
  parts.removeLast(); // Remove filename to leave only directories

  final List<String> relParts = relativeImport.split('/');
    ...
    if (entity is File && entity.path.endsWith('.dart')) {
      ...
      if (importRegex.hasMatch(content)) {
        content = content.replaceAllMapped(importRegex, (match) {
          ...
          final String absPkgPath = mapOldPackagePath(
      ...
      if (packageImportRegex.hasMatch(content)) {
        content = content.replaceAllMapped(packageImportRegex, (match) {
          final String pkgPath = match.group(1)!;
          final String mappedPkgPath = mapOldPackagePath(pkgPath);
