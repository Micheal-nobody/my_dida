...
class _HabitDataSummaryPageState extends State<HabitDataSummaryPage>
  ...
  Widget _buildDaySummary() {
    ...
    final completedCount =
        activeHabits.where((h) => provider.isTodayCompleted(h)).length;
            ...
            return ListTile(
              ...
              title: Text(habit.name,
                  style: TextStyle(
                      ...
                      decoration:
                          habit.isTodaySkipped ? TextDecoration.lineThrough : null)),
              subtitle: Text(habit.isTodaySkipped
                  ? '已跳过今天'
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final habitName = activeHabits
                        .firstWhere((h) => h.id == record.habitId,
                            orElse: () => Habit(
                                  name: '未知习惯',
                                  icon: '',
  ...
  Widget _buildWeekSummary() {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (activeHabits.isNotEmpty) {
        ...
        int targetTotal =
            activeHabits.map((e) => e.checkInCount).reduce((a, b) => a + b);
        weeklyCompletionRate[i] =
            targetTotal == 0 ? 0.0 : (checkInCount / targetTotal).clamp(0.0, 1.0);
      }
    }
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      trailing: Text('本周打卡 ${entry.value} 次',
                          style: const TextStyle(color: Colors.grey)),
                    );
                  },
                ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              const Text('少  ', style: TextStyle(fontSize: 11, color: Colors.grey)),
              ...
              const Text('  多', style: TextStyle(fontSize: 11, color: Colors.grey)),
            ],
          ),
        ],
      ),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < 7; r++) {
      if (weekdayLabels[r].isNotEmpty) {
        ...
        textPainter.paint(
          canvas,
          Offset(2, r * (sizeBox + spacing) + 10 + (sizeBox - textPainter.height) / 2),
        );
      }
    }
  }
