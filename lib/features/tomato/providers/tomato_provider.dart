...
class TomatoProvider with ChangeNotifier {
  ...
  Future<void> _handleTomatoEvent(TomatoEvent event) async {
    if (event is TomatoTickEvent) {
      ...
      final record = TomatoRecord(
        ...
        taskName: _activeCustomTomato != null
            ? _activeCustomTomato!.name
            : _associatedTask?.name,
        categoryName: _activeCustomTomato != null
            ? '自定义番茄钟'
            : await _getChecklistName(_associatedTask?.checklistId),
  ...
  Future<Map<String, dynamic>> getCustomTomatoTotalStats(
    int customTomatoId,
  ) async {
    ...
    return {'completedCount': completedCount, 'totalMinutes': totalMinutes};
  }
