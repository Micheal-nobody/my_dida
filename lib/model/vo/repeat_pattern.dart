...
class RepeatPattern {
  ...
  DateTime? nextOccurrenceAfter(DateTime startTime, DateTime anchor) {
    ...
    if (isEbbinghaus) {
      ...
      for (final offset in offsets) {
        ...
        if (occurrence.isAfter(normalizedAnchor)) {
          return DateTime(occurrence.year, occurrence.month, occurrence.day, startTime.hour, startTime.minute);
        }
      }
      return null;
    }
    ...
    if (isWorkday) {
      ...
      while (cursor.weekday == DateTime.saturday || cursor.weekday == DateTime.sunday) {
        cursor = cursor.add(const Duration(days: 1));
      }
      return DateTime(cursor.year, cursor.month, cursor.day, startTime.hour, startTime.minute);
    }
    ...
    if (more.isNotEmpty) {
      return DateTime(more.first.year, more.first.month, more.first.day, startTime.hour, startTime.minute);
    }
