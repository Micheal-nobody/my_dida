...
void main() {
  ...
  group('Habit Undo Consistency Tests', () {
    test('Check-in and skip operations can be recorded and fully undone with records removed', () async {
      // 1. 创建习惯
      final habit = Habit(
        name: 'Drink Water',
        icon: 'water',
      ...
      final recordsAfterCheckIn = await recordRepository.getRecordsByHabitId(savedHabit.id);
      ...
      // 验证撤销后，打卡记录也被删除了
      final recordsAfterUndo = await recordRepository.getRecordsByHabitId(savedHabit.id);
      ...
      final recordsAfterSkip = await recordRepository.getRecordsByHabitId(savedHabit.id);
      expect(recordsAfterSkip.length, 1);
      expect(recordsAfterSkip.first.isSkip, true);

      // 5. 撤销跳过打卡
      await operationStack.undo();
      final unskippedHabit = await habitRepository.getHabitById(savedHabit.id);
      expect(unskippedHabit!.isTodaySkipped, false);

      // 验证撤销后，skip 记录也被删除
      final recordsAfterUndoSkip = await recordRepository.getRecordsByHabitId(savedHabit.id);
      expect(recordsAfterUndoSkip, isEmpty);
    });

    test('Archive and unarchive operations can be recorded and undone', () async {
      final habit = Habit(
        name: 'Read Book',
        icon: 'book',
        remindTime: DateTime.now(),
      ...
      final unarchivedHabit = await habitRepository.getHabitById(savedHabit.id);
    ...
    test('Delete habit operation and all check-in records can be fully undone', () async {
      // 1. 创建习惯
      final habit = Habit(
        name: 'Gym',
        icon: 'gym',
      ...
      final reloadedHabit = await habitRepository.getHabitById(savedHabit.id);
      final records = await recordRepository.getRecordsByHabitId(savedHabit.id);
      ...
      final deletedRecords = await recordRepository.getRecordsByHabitId(savedHabit.id);
      ...
      final restoredRecords = await recordRepository.getRecordsByHabitId(savedHabit.id);
